...
export default function SearchResultItem({ item, handleRedirect }: { item: any, handleRedirect: (href: string) => void }) {
    return (
        <Button type="button" className="w-full flex items-center gap-4 justify-between font-bold border border-(--color-light-gray) rounded-lg bg-(--color-white) p-4 shadow-md" onClick={() => handleRedirect(item.href)}>
            <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col w-full items-start gap-1">
                    ...
                    <p className="text-sm text-(--color-dark-blue) text-left line-clamp-2">
                        {item.name}
                    </p>
                    <p className="text-xs text-(--color-gray) text-left line-clamp-2 font-light">
                        {item.description}
                    </p>
                </div>
            </div>
