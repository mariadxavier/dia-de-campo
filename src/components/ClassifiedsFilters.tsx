...
export default function ClassifiedsFilters({
  ...
  return (
    <section className="flex flex-col w-full gap-4 md:gap-6 py-2 px-5 md:p-8 lg:p-16">
      <div className="flex flex-col gap-1">
        <h1 className="font-bold text-2xl text-(--color-dark-blue) lg:text-3xl">Encontre o anúncio certo</h1>
        <p className="text-(--color-gray) text-sm lg:text-base">
          Filtre por categoria e localização.
        </p>
      </div>
      <div className='flex flex-col gap-4 md:gap-6 lg:flex-row lg:gap-18'>
        ...
        <div className="flex flex-col gap-2 rounded-lg bg-(--color-white-dust) border border-(--color-light-gray) p-3 lg:p-6 lg:w-1/2 max-w-[500px]">
          <p className="font-semibold txt-(--color-dark-blue) text-sm">Localização</p>
          <div className="cursor-pointer bg-(--color-light-green) max-w-[200px] px-4 py-2 border border-(--color-green) rounded-full text-(--color-green)">
            <select
